...
namespace AssetBundleBrowser
{
    ...
    public class AssetBundleManageTab 
    {
        ...
        internal AssetBundleTree m_BundleTree;
        ...
        internal ManageTabData m_ManageData;

        [System.Serializable]
        internal class ManageTabData
        {
            internal Dictionary<string, bool> m_Enables;
        }

        internal AssetBundleManageTab()
        {
            m_ManageData = new ManageTabData();
            m_ManageData.m_Enables = new Dictionary<string, bool>();
        ...
        internal void OnEnable(Rect pos, EditorWindow parent)
        {
            ...
            dataPath += "/Library/AssetBundleManageBuild.dat";

            if (File.Exists(dataPath))
            {
                BinaryFormatter bf = new BinaryFormatter();
                FileStream file = File.Open(dataPath, FileMode.Open);
                var data = bf.Deserialize(file) as ManageTabData;
                if (data != null)
                    m_ManageData = data;
                file.Close();
            }
        ...
        internal void OnDisable()
        {
            ...
            bf.Serialize(file, m_ManageData);
            file.Close();
        ...
        internal void OnGUI(Rect pos)
        {
            ...
            if(m_BundleTree == null)
            {
                ...
                var rows = m_BundleTree.m_Controller.m_BundleTree.GetRows();
                foreach (BundleTreeItem row in rows) 
                {
                    if (m_ManageData.m_Enables.TryGetValue(row.displayName, out var enable))
                        row.m_Enable = enable;
                }
            }
